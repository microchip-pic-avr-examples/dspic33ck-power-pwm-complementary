...
volatile P33C_PWM_GENERATOR_t my_pg;    // user-defined PWM generator object 
...
volatile uint16_t PWM_Initialize(void) {
    ...
    p33c_PwmGenerator_ConfigWrite(my_pg.Instance, pgConfigDispose);

    // Set individual PWM generator configuration
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    
    // PGxCONL: PWM GENERATOR x CONTROL REGISTER LOW
    my_pg.pgHandle->PGxCONL.bits.CLKSEL = 0b01; // PWM Generator uses Master clock selected by the MCLKSEL[1:0] (PCLKCON[1:0]) control bits
    my_pg.pgHandle->PGxCONL.bits.MODSEL = 0b000; // Independent Edge PWM mode
    my_pg.pgHandle->PGxCONL.bits.HREN = 1; // PWM Generator x operates in High-Resolution mode
    
    // PGxCONH: PWM GENERATOR x CONTROL REGISTER HIGH
    my_pg.pgHandle->PGxCONH.bits.UPDMOD = 0b00; // SOC update: Data registers at start of next PWM cycle if UPDREQ = 1
    my_pg.pgHandle->PGxCONH.bits.TRGMOD = 0b0; // PWM Generator operates in Single Trigger mode
    my_pg.pgHandle->PGxCONH.bits.SOCS = 0b0000; // SOCS[3:0]: Start-of-Cycle Selection: Local EOC ? PWM Generator is self-triggered
    
    // PGxIOCONL: PWM GENERATOR x I/O CONTROL REGISTER LOW
    my_pg.pgHandle->PGxIOCONL.bits.OSYNC = 0b00; // User output overrides via the OVRENH/L and OVRDAT[1:0] 
                                                 // bits are synchronized to the local PWM time base (next Start-of-Cycle)
    my_pg.pgHandle->PGxIOCONL.bits.OVRDAT = 0b00; // Both PWM outputs are LOW in override mode
    my_pg.pgHandle->PGxIOCONL.bits.OVRENL = 1; // OVRDAT0 provides data for output on the PWMxL pin
    my_pg.pgHandle->PGxIOCONL.bits.OVRENH = 1; // OVRDAT1 provides data for output on the PWMxH pin
        
    // PGxIOCONH: PWM GENERATOR x I/O CONTROL REGISTER HIGH
    my_pg.pgHandle->PGxIOCONH.bits.PMOD = 0b00; // PWM Generator outputs operate in Complementary mode
    
    // Set PWM signal generation timing of this generator 
    
    my_pg.pgHandle->PGxPER.value = 20000;   // Set Period (n = 20000 => 1 / (n * 250 ps) = 200 kHz )
    my_pg.pgHandle->PGxDC.value  = 5000;    // Set Duty Cycle (n = 5000 => n / PGxPER = 25.0 %)
    my_pg.pgHandle->PGxDTH.value = 400;     // Set Rising Edge Dead Time (n = 400 => n * 250 ps = 100 ns)
    my_pg.pgHandle->PGxDTL.value = 600;     // Set Falling Edge Dead Time (n = 600 => n * 250 ps = 150 ns)
    ...
    // Check return value
    retval = (bool)(my_pg.pgHandle->PGxCONL.bits.ON) &&         // Check if PWM generator is turned on
                   (!my_pg.pgHandle->PGxIOCONL.bits.OVRENH);    // Check if PWM generator outputs are active
